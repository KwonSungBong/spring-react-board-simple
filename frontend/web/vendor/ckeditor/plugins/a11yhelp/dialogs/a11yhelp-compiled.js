...
CKEDITOR.dialog.add("a11yHelp", function (l) {
  var a = l.lang.a11yhelp,
    ...
    var a = l.keystrokeHandler.keystrokes,
      ...
      var b;if (g[c]) {
        b = g[c];for (var h, k, m = [], d = 0; d < f.length; d++) {
          k = f[d], h = b / f[d], 1 < h && 2 >= h && (b -= k, m.push(e[k]));
        }m.push(e[b] || String.fromCharCode(b));b = m.join("+");
      } else b = a;return b;
    };
  }();return { title: a.title, minWidth: 600, minHeight: 400, contents: [{ id: "info", label: l.lang.common.generalTab, expand: !0, elements: [{ type: "html", id: "legends", style: "white-space:normal;", focus: function focus() {
        ...
        html: function () {
          for (var e = '\x3cdiv class\x3d"cke_accessibility_legend" role\x3d"document" aria-labelledby\x3d"' + n + '_arialbl" tabIndex\x3d"-1"\x3e%1\x3c/div\x3e\x3cspan id\x3d"' + n + '_arialbl" class\x3d"cke_voice_label"\x3e' + a.contents + " \x3c/span\x3e", g = [], c = a.legend, l = c.length, f = 0; f < l; f++) {
            for (var b = c[f], h = [], k = b.items, m = k.length, d = 0; d < m; d++) {
              var q = k[d],
                  r = q.legend.replace(p, t);r.match(p) || h.push("\x3cdt\x3e%1\x3c/dt\x3e\x3cdd\x3e%2\x3c/dd\x3e".replace("%1", q.name).replace("%2", r));
            }g.push("\x3ch1\x3e%1\x3c/h1\x3e\x3cdl\x3e%2\x3c/dl\x3e".replace("%1", b.name).replace("%2", h.join("")));
